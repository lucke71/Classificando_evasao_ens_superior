create table Unb_grupo_ingressantes as (
	with alunos_2010 as(
		select 	distinct 
				t1.co_aluno,
				t1.nu_cpf,
		...
		where t1.co_ies=2 and t1.co_modalidade_ensino=1 and t1.co_nivel_academico=1
			and extract(year from dt_ingresso_curso)=2010 and tp_atributo_ingresso=0
	),
	...
	alunos_2011 as (
		...
		where t1.co_ies=2 and t1.co_modalidade_ensino=1 and t1.co_nivel_academico=1
			and extract(year from dt_ingresso_curso)=2011 and tp_atributo_ingresso=0
	),
	...
	alunos_2012 as (
		...
		where t1.co_ies=2 and t1.co_modalidade_ensino=1 and t1.co_nivel_academico=1
			and extract(year from dt_ingresso_curso)=2012 and tp_atributo_ingresso=0
	),
	...
	alunos_2013 as (
		...
		where t1.co_ies=2 and t1.co_modalidade_ensino=1 and t1.co_nivel_academico=1
			and extract(year from dt_ingresso_curso)=2013 and tp_atributo_ingresso=0
	),
	...
	alunos_2014 as (
		...
		where t1.co_ies=2 and t1.co_modalidade_ensino=1 and t1.co_nivel_academico=1
			and extract(year from dt_ingresso_curso)=2014 and tp_atributo_ingresso=0
	),
	...
	evasao as (
		...
		from(
			select 	t1.*,
					count(co_ocde_area_geral) over (partition by co_aluno,ano,co_ocde_area_geral) as num_grupos,
					count(case when co_aluno_situacao=4 then 1 end) over (partition by co_aluno,ano,co_ocde_area_geral) as num_desvinc,
					count(case when co_aluno_situacao=5 then 1 end) over (partition by co_aluno,ano,co_ocde_area_geral) as num_transf
			from unb t1
		)tab1
